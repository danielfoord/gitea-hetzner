...
- `gitea-values.yaml` declares no passwords at all — the Gitea admin password/email go in the gitignored `secrets.local.yaml` (see `secrets.local.yaml.example`); the Postgres password is left unset entirely and auto-generated by the `postgresql` subchart, retrievable from its Secret if ever needed by hand. Everything else sensitive (B2/restic keys, runner registration token) is created directly with `kubectl create secret` — never committed inline.
